...
> 특히 객관식(MC)은 자유 generation engine을 써서 공식 제약 디코딩과 **미정렬**입니다.
> 자세한 내용과 한계는 [docs/03-gsma-alignment-and-caveats.md](docs/03-gsma-alignment-and-caveats.md).

## 5분 Quick Start

```bash
# 0) 환경 준비 (GPU 서버) — lm-eval@97a5e2c7 + gsma-evals clone, .venv 구성
...
- bare `open_telco_otlite` / `open_telco_otfull`은 **실행 불가**입니다(run 스크립트 fail-fast, exit 2).
...
이 저장소는 내부 baseline harness입니다. 멀티모달/LMM/LAM, Planning, RAG, 한국어 QA 등은
범위 밖(2차 과제)입니다. 전체 개발 이력(PR/commit/실험)은 원본 engineering-source 저장소가
보존합니다.
